...
// =====Main functions=====
int find_employee(const struct Employee *employees, const int size);
...
int main(){
    ...
    to_string(employees[find_employee(employees, size)]);
...
int find_employee(const struct Employee *employees, const int size){
    ...
    for(int i = 0; i < size; i++)
        if(employees[i].birthyear > employees[max_index].birthyear)
            max_index = i;
    return max_index;
}
