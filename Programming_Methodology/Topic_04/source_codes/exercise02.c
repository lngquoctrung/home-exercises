...
// ==== Main function ====
int find_max_value(int*, int);

int main(){
    // Write function to find the maximum number of an integer array
    ...
    /*
    Initialize array elements by entering values manually
    ...
    */
    ...
    // Find maximum number of integer array
    printf("\nThe maximum number of array is %d", find_max_value(arr, arr_size));
    return 0;
}
...
// === Main function ===
int find_max_value(int *array, int arr_size){
    int max = 0;
    for(int i = 0; i < arr_size; i++){
        if(array[i] > max){
            max = array[i];
