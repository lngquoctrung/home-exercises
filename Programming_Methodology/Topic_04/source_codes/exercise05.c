...
// ==== Main function ====
int sum_non_pos_nums(int*, int);

int main(){
    // Write function to sum all non-positive numbers of an integer array
    ...
    // Print sum result all non-positive numbers of an integer array
    printf("\nThe result of sum all non-positive numbers of array = %d", sum_non_pos_nums(arr, arr_size));
    return 0;
}
...
// === Main function ===
int sum_non_pos_nums(int *array, int arr_size){
    int non_pos_sum = 0;
    for(int i = 0; i < arr_size; i++){
        if(array[i] < 0){
            non_pos_sum += array[i];
